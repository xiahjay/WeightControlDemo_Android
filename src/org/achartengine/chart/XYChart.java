...
public abstract class XYChart extends AbstractChart {
  ...
  /** The visible chart area, in screen coordinates. 可见图区域,在屏幕坐标 */
   ...
   * @param x the top left x value of the view to draw to 左上角的x值点
   * @param y the top left y value of the view to draw to 左上角的y值点
   * @param width the width of the view to draw to View的宽度
   * @param height the height of the view to draw to View的高度
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    // use a linked list for these reasons: 使用链表对这些原因:
    // 1) Avoid a large contiguous memory allocation 1)避免大量连续的内存分配
    // 2) We don't need random seeking, only sequential reading/writing, so  2)我们不需要随机寻找,只顺序读/写,所以
    // linked list makes sense 链表是有意义的
